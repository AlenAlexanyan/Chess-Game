...
bool isValidMove(char board[8][8], int fromRow, int fromCol, int toRow, int toCol)
{
    ...
    else if (!isBlackFigure(board, fromRow, fromCol) || isBlackFigure(board, toRow, toCol) ||
             fromRow < 0 || fromRow >= 8 || fromCol < 0 || fromCol >= 8 ||
             toRow < 0 || toRow >= 8 || toCol < 0 || toCol >= 8)
    {
        // std::cout << "First Condition: " << !isBlackFigure(board, fromRow, fromCol) << std::endl;
        // std::cout << "Second Condtion: " << isBlackFigure(board, toRow, toCol) << std::endl;
...
{   
    bool bishopMove = isPosibleMoveForBishop(board, fromRow, fromCol, toRow, toCol);
    bool rookMove = isPosibleMoveForRook(board, fromRow, fromCol, toRow, toCol);
...
bool isPosibleMove(char board[8][8],  int fromRow, int fromCol, int toRow, int toCol) {
    switch (board[fromRow][fromCol]){
        case 'p': case 'P':
            ...
            break;
        case 'b': case 'B':
            return isPosibleMoveForBishop(board, fromRow, fromCol, toRow, toCol);
            break;
        case 'n': case 'N':
            return isPosibleMoveForKnight(board, fromRow, fromCol, toRow, toCol);
            break;
        case 'q': case 'Q':
            return isPosibleMoveForQueen(board, fromRow, fromCol, toRow, toCol);
            break;
        case 'k': case 'K':
            return isPosibleMoveForKing(board, fromRow, fromCol, toRow, toCol);
            break;
        default:
            return false;
            break;
...
bool isCorrectFigure(char board[8][8], int row, int col, bool isWhite) {
    ...
    bool val = (isWhite && piece >= 'A' && piece <= 'Z') || (!isWhite && piece >= 'a' && piece <= 'z');
    // std::cout << "IsWhite: " << isWhite << std::endl;
    // std::cout << (isWhite && piece >= 'A' && piece <= 'Z') << std::endl;
    // std::cout << (!isWhite && piece >= 'a' && piece <= 'z') << std::endl;
    // std::cout<< "Return valueL " << val << std::endl;
    return val;
...
bool isCheck(char board[8][8])
{
    ...
    std::cout << "White King" << ' ' << WhiteKing.first << ' ' << WhiteKing.second << std::endl;
    std::cout << "Black King" << ' ' << BlackKing.first << ' ' << BlackKing.second << std::endl;

    // Check for attacks from different pieces
    for (size_t i = 0; i < 8; i++) {
        for (size_t j = 0; j < 8; j++) {
            // std::cout<< "Move Check: " << black_or_white << std::endl;
            char piece = board[i][j];
            std::cout << "i and j" << ' '<< i << ' ' << j << ' ' << board[i][j] << std::endl;
    ...
    std::cout << "It is not a check." << std::endl;
...
bool isCheckMate(char board[8][8])
{
    ...
    // std::cout << "White King Coordinates: " << checkKing.first << ' ' << checkKing.second << std::endl;
    ...
    for (int row = 0; row < 8; ++row) {
        for (int col = 0; col < 8; ++col) {
            if (isCorrectFigure(board, row, col, whiteCheck)) {
                for (auto &field : closingFields) {
                    // std::cout << "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa" << ' ' << row << ' ' << col << ' ' << field.first << ' ' << field.second << std::endl;
